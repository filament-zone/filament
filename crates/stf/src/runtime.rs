...
///    the module state, but if the call throws an error, no state is updated (the transaction is
///    reverted).
///
/// `#[derive(MessageCodec)]` adds deserialization capabilities to the `Runtime` (by implementing
/// the `decode_call` method). `Runtime::decode_call` accepts a serialized call message and returns
/// a type that implements the `DispatchCall` trait.  The `DispatchCall` implementation (derived by
/// a macro) forwards the message to the appropriate module and executes its `call` method.
...
pub struct Runtime<S: Spec, Da: DaSpec> {
    ...
    /// The sequencer registry module is responsible for authorizing users to sequencer rollup
    /// transactions
